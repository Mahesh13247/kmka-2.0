...
export const VideoGrid: React.FC<VideoGridProps> = ({ videos, onSelectVideo, isLoading, error, lastVideoElementRef, toggleFavorite, favorites, viewMode }) => {
    if (error) {
    ...
    return (
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-6">
            {videos.map((video, index) => (
                <div
                    key={`${video.id}-${index}`}
                    ref={lastVideoElementRef && videos.length === index + 1 ? lastVideoElementRef : null}
            ...
            {isLoading && Array.from({ length: 10 }).map((_, i) => <LoadingSkeleton key={i} />)}
        </div>
    );
};